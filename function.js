window.function = async function(api_key, vector_store_id, file_batch_id) {
    // Validate API Key
    if (!api_key.value) {
    ...
    if (!file_batch_id.value) {
        return "Error: File Batch ID is required.";
    }

    // API endpoint URL
    const apiUrl = `https://api.openai.com/v1/vector_stores/${vector_store_id.value}/file_batches/${file_batch_id.value}`;
